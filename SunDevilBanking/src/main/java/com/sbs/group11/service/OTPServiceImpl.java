 ...
 *
 * USAGE : To generate OTP invoke the generateOTP method with its arguments 
 * secretcode - sessionID or shared secret between server and client (not a constant),
 * dynamicNumber - a dynamic number which changes on a per use basis like time or counter,
 * otpLength - length of the OTP which can be 8 digits max,
 * checksumFlag - tells if checksum needs to be added to OTP,
 * terminalOffset - Offset into MAC to begin truncation. It needs to be dynamic on a per use basis (can use the dynamicNumber argument value)
 * 
...
public class OTPServiceImpl implements OTPService {
    ...
    public  byte[] hmac_sha512(byte[] hmacbytes, byte[] authenticateText)
        ...
        SecretKeySpec macKey =
                new SecretKeySpec(hmacbytes, "RAW");
        hmacSha512.init(macKey);
        return hmacSha512.doFinal(authenticateText);
    }
    
    
     public String generateOTP(byte[] secretcode,
            long dynamicNumber,
            int otpLength,
